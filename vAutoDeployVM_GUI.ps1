...
function connectServer{
    ...
    $now = Get-Date -format "dd-MMM-yyyy HH:mm | "
    $outputTextBox.text = "`r`n$now Something went wrong!!"
...
function ButtonValidateDatastoreAction {
 ...
 try {
	...
	$VirtualPortGroups = Get-virtualportgroup | sort-object name
	$now = Get-Date -format "dd-MMM-yyyy HH:mm | "
	$outputTextBox.text = "`r`n$now Seleccionado datastore $($DropDownBoxDatastore.SelectedItem.ToString())" + $outputTextBox.text
	$outputTextBox.text = "`r`n$now Cargada lista de port groups" + $outputTextBox.text
...
write-host Desplegando $vmname con IP $ip de $NumVMs servers -foregroundcolor yellow
...
#-------------GROUP BOX DATOS DE CONEXIÓN-----------------------------
	...
	$TextBoxIPorFQDN.Text = "172.28.10.19"
...
#	$RadioButtonTemplate.Checked = $true #is checked by default
	...
	$RadioButtonVM.Location = new-object System.Drawing.Point(100,95) #location of the radio button(px) in relation to the group box's edges (length, height)
